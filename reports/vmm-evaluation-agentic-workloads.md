...
### 1. Cloud Hypervisor (current — containerd-cloudhypervisor v0.7.0)

Rust-based VMM from Intel/Microsoft, run as a separate process per VM. Our
shim spawns it, communicates via HTTP API socket.

| Attribute | Value |
|-----------|-------|
| Language | Rust (rust-vmm crates) |
| Process model | 1 CH process + 1 shim per pod (2 total) |
| VMM RSS overhead | ~5-10 MB (excluding guest memory) |
| Per-pod overhead (RuntimeClass) | 10m CPU, 50Mi memory |
| Boot time | ~350ms e2e (lazy boot: CH spawned at sandbox, VM booted at container start) |
| Disk hot-plug | ✅ vm.add-disk HTTP API (first disk pre-attached at boot) |
...
HTTP management API. Combined with our minimal guest image (16MB rootfs, no
virtiofsd, no systemd), achieves **~59 MB actual host RSS per pod** — ~5.6×
lower than Kata's ~330 MB on identical hardware. Devmapper passthrough +
lazy boot + pre-attached rootfs + RunContainer RPC.

**Weaknesses:** Separate VMM process per VM. HTTP API socket adds ~5ms latency
per call. Two processes per pod. CPU at 150-pod scale is 46-49%, ~2.5× higher
than Kata (16-22%), driven by per-pod process overhead.
...
| | Our CH Shim | Kata v3 Dragonball | Kata v2 (AKS) | libkrun | krun | OpenVMM |
|---|---|---|---|---|---|---|
| **Processes/pod** | 2 | **1** | 3 | **1** | 1 | 2 |
| **VMM RSS** | 5-10 MB | **~5 MB** | 25-50 MB | ~5 MB | ~5 MB | ? |
| **Per-pod host RSS** | **~59 MB** | ~50-100MB* | **~330 MB** | ~50MB* | N/A | ? |
| **Pod overhead** | **50Mi** | ~50-100Mi* | 600Mi | ~30Mi* | N/A | ? |
| **Boot (sandbox)** | **97ms** | <200ms | 500-1000ms | <50ms* | N/A | ? |
...
| **Built-in VMM** | ❌ | **✅** | ❌ | **✅** | ✅ | ❌ |
| **Confidential** | ✅ | ⚠️ | ✅ | ✅ | ✅ | ✅ |
| **150-pod test** | **150/150** | Untested | 130/150 | Untested | N/A | Untested |

*Projected, not measured

## Analysis: Kata v3 Dragonball Changes the Picture

Our original VMM evaluation overlooked the most important development in the
container isolation space: **Kata v3 already solved the "VMM as library"
problem** with Dragonball, and did it with full device hot-plug via upcall.
...
**Kata v3 with Dragonball is the most promising path for the ecosystem.** It
solves the "VMM as library" problem with production-tested upcall hot-plug,
...
placement) are directly portable to Kata v3's architecture. Contributing them
upstream while working on MSHV for Dragonball would create the ideal solution:
Kata's production orchestration + our density optimizations + Azure
compatibility.

For the immediate future, our shim (v0.7.0) remains the best option for Azure
deployments — it's the only solution achieving 150/150 pods with full metrics
on AKS. But the long-term play is to bring these capabilities to Kata v3.

---

*Analysis based on containerd-cloudhypervisor v0.7.0 benchmarks on AKS
(westus3, D8ds_v5 nodes, pod spec: 100m CPU / 64Mi req / 256Mi limit),
